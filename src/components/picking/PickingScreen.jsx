  ...
  const AnimatedExclamation = () => (
    <span className="inline-flex ml-1">
      <motion.span
        ...
        className="inline-block text-blue-400 font-bold"
      >
        !
      </motion.span>
    </span>
...
const PickingScreen = memo(({ order, onComplete, initialPickedItems = [], resetSelection = false, onBack, shouldUndoLastPick = false, onUndoLastPickComplete, initialTimerSeconds = 0 }) => {
  ...
  const { completedItems, incompleteItems } = useMemo(() => {
    ...
    // Sort both arrays by bin location
    completed.sort((a, b) => a.binLocation.localeCompare(b.binLocation));
    incomplete.sort((a, b) => a.binLocation.localeCompare(b.binLocation));
  ...
  useEffect(() => {
    window.pickingPageData = {
      ...
      onBack: onBack,
      isPickingScreen: true // Flag to indicate we're in the actual picking screen
    };
  ...
  }, [order?.orderId, order?.id, order?.salesOrderNumber, order?.soNumber, order?.customer, 
        ...
        return (
          <>
            <div className="flex items-center space-x-3">
              ...
              <h3 className="text-4xl font-bold text-white">Metrics & Guide</h3>
              <div className="bg-purple-500/20 border border-purple-500/50 px-3 py-1 rounded-full">
                <span className="text-purple-300 font-bold">Stats & Tiers</span>
              </div>
            </div>
          </>
        );
            ...
            (() => {
              ...
              return (
                ...
                <div className="bg-white/[0.03] backdrop-blur-xl rounded-xl p-6 text-center border border-white/10">
                  <div className="flex justify-center mb-3">
                    {renderMixedGameIcons(order.items, totalPicked, 96)}
                  </div>
                  <p className="text-4xl font-bold">{totalPicked}</p>
              ...
              <div className={`flex-1 bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/30 rounded-xl p-4 shadow-lg shadow-purple-500/20 flex flex-col ${isInBottomZone ? 'min-h-[400px]' : 'min-h-0'}`}>
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4 content-center items-center min-h-0">
                  <div className="flex items-center space-x-3">
                    {renderMixedGameIcons(order.items, 1, 144, true)}
                    <span className="text-white text-4xl font-bold">= 1</span>
                  </div>
                  
                  <div className="flex items-center space-x-3">
                    {renderMixedGameIcons(order.items, 5, 144, true)}
                    <span className="text-white text-4xl font-bold">= 5</span>
                  </div>
                  
                  <div className="flex items-center space-x-3">
                    {renderMixedGameIcons(order.items, 25, 144, true)}
                    <span className="text-white text-4xl font-bold">= 25</span>
                  </div>
                  
                  <div className="flex items-center space-x-3">
                    {renderMixedGameIcons(order.items, 125, 144, true)}
                    <span className="text-white text-4xl font-bold">= 125</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <motion.div 
              ...
              >
                ...
                {dragOverZone === 'top' && draggingSection && dropzoneContents['top'] !== draggingSection && (
                  ...
                  >
                    <div className={`${
                      draggingSection === 'itemsToPick' ? 'bg-blue-500/20 border-blue-400' :
